...
test.describe("Admin Login and Permissions Workflow", () => {
	...
	test("check if invalid email id shows a toast error and stays on login page", async ({
		page,
	}) => {
		await page.goto("/");
	...
	test("check if invalid password shows a toast error and stays on login page", async ({
		page,
	}) => {
		await page.goto("/");
		await page.getByRole("link", { name: /login/i }).click();
	...
	test("verify non-admin users cannot access Admin Dashboard", async ({ page }) => {
		await loginAsUser(page);
		await page.goto("/dashboard/admin");
		await expect(page).toHaveURL(/\/login/i);

		await page.goto("/");
		await expect(page.getByRole("button", { name: /normal user/i })).toBeVisible();
	});

	test("verify non-admin users should not have access to Admin controls", async ({ page }) => {
		await loginAsUser(page);
		await page.getByRole("button", { name: /normal user/i }).click();
